...
bool checkTag(const char* tag, const char *in, int argc, char **argv, FILE *newFile)
{
    char buffer[100];
    sprintf(buffer, "%s=", tag);

    if (!strncmp(in, buffer, strlen(buffer))) {
		
		fprintf(newFile, "%s\n", in);
		
		for (int i = 2; i < argc; i++) {
			const char *test = k_dcgettext(argv[0], in, argv[i]);
			
			if (!strncmp(in + 1, buffer, strlen(buffer))) {
				fprintf(stderr, "ERROR: translation of %s \"%s\" contains tag not at the beginning of line\n", argv[i], test);
				continue;
			}
			
			if (!strcmp(test, buffer) || !strcmp(test, in))
				continue;
			if (strncmp(test, tag, strlen(tag))) {
				fprintf(stderr, "ERROR: %s's translation of \"%s\" doesn't fit tag\n", argv[i], in);
				continue;
			}
			if (strchr(test, '\n')) {
				...
				char out[3000];
				sprintf(out, "%s[%s]=%s", tag, argv[i], test + strlen(tag) + 1);
				// out[strlen(out) - strlen(tag) - 1] = 0;
				fprintf(newFile, "%s\n", out);
			}
			return true;
		}
